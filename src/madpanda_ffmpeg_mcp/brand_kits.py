...
def sanitize_brand_kit(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    kit_id = validate_brand_kit_id(payload.get("brand_kit_id"))
    ...
    if payload.get("logo_position"):
        cleaned["logo_position"] = sanitize_position(payload["logo_position"], LOGO_POSITIONS)
    else:
    ...
    if payload.get("logo_opacity") is not None:
        cleaned["logo_opacity"] = sanitize_opacity(payload["logo_opacity"], DEFAULT_LOGO_OPACITY)
    else:
    ...
    if payload.get("text_position"):
        cleaned["text_position"] = sanitize_position(payload["text_position"], TEXT_POSITIONS)
    else:
    ...
    if payload.get("caption_highlight_color"):
        cleaned["caption_highlight_color"] = sanitize_color(
            payload["caption_highlight_color"], "yellow"
        )
    ...
    validate_brand_kit_record(cleaned)
    return cleaned
