...
from .storage import (
    ...
    job_storage_budget,
    persist_asset,
    purge_current_job_outputs,
)
from .templates import get_template, validate_template_variables
from .tenant import require_owner_hash, tenant_context
...
def _create_output_asset(
    ...
    expected_kind = kind_from_mime_type(mime_type)
    if expected_kind is None:
        raise JobError("Output media type is unsupported")
    try:
        validated_probe = validate_media_file(
            path,
            expected_kind=expected_kind,
            byte_limit=settings.max_output_bytes,
        )
    except MediaLimitError as exc:
        raise JobError("Output media exceeds a configured limit") from exc
    ...
    asset = {
        "asset_id": asset_id,
        "source": "process",
        "original_filename": f"{asset_id}.{extension.removeprefix('.')}",
        "mime_type": mime_type,
        "sha256": sha256,
    ...
    asset.update(validated_probe)
    return persist_asset(path, asset, extension, count_as_job_output=True)
...
def _probe_optional(path: str) -> dict[str, Any] | None:
    try:
        ...
        return validate_media_probe(probe)
    except Exception:
        return None
...
def _resolve_input_path(asset: dict[str, Any]) -> tuple[str, bool]:
    ...
    cleanup = settings.storage_backend == "s3"
    expected_kind = kind_from_mime_type(asset.get("mime_type"))
    if expected_kind is None:
        if cleanup and os.path.exists(temp_path):
            os.remove(temp_path)
        raise JobError("Input asset media type is unsupported")
    try:
        validate_media_file(
            temp_path,
            expected_kind=expected_kind,
            byte_limit=max(settings.max_ingest_bytes, settings.max_output_bytes),
        )
    except MediaLimitError as exc:
        if cleanup and os.path.exists(temp_path):
            os.remove(temp_path)
        raise JobError("Input media exceeds a configured limit") from exc
...
def _finish_job(job_id: str, status: str, updates: dict[str, Any]) -> None:
    ...
    updates = _sanitize_job_updates(updates)
    updates = {**updates, "status": status, "updated_at": utc_now_iso()}
    update_job(job_id, updates)


_URL_PATTERN = re.compile(r"(?i)\b(?:https?|s3)://\S+")
_ABSOLUTE_PATH_PATTERN = re.compile(r"(?<![A-Za-z0-9_.-])(?:/[A-Za-z0-9_.@%+=:,~-]+){2,}")
_WINDOWS_PATH_PATTERN = re.compile(r"(?i)\b[A-Z]:\\(?:[^\s\\]+\\)+[^\s\\]+")
_CREDENTIAL_PATTERN = re.compile(
    r"(?i)\b(?:authorization|token|secret|password|api[-_]?key|credential)\b\s*[:=]\s*\S+"
)
_FORBIDDEN_JOB_KEYS = {
    "args",
    "arguments",
...
def _safe_error(value: Any) -> tuple[str, str]:
    ...
    if "timeout" in lowered or "timed out" in lowered:
        return "timeout", "Processing timed out"
    if any(term in lowered for term in ("exceed", "too large", "limit", "quota")):
        return "limit_exceeded", "Configured processing limit exceeded"
    if "not found" in lowered or "missing" in lowered:
        return "not_found", "Required media resource was not found"
    if "allowlist" in lowered or "not allowed" in lowered or "disabled" in lowered:
        return "not_allowed", "Operation is not allowed by server policy"
    if "invalid" in lowered or "required" in lowered:
        return "invalid_request", "Processing input is invalid"
    return "processing_error", "Media processing failed"


def _sanitize_job_updates(updates: dict[str, Any]) -> dict[str, Any]:
    sanitized = {
        key: value
        for key, value in updates.items()
        if key.lower() not in _FORBIDDEN_JOB_KEYS
        and "traceback" not in key.lower()
        and "credential" not in key.lower()
    }
    if "error" in sanitized and sanitized["error"] is not None:
        code, message = _safe_error(sanitized["error"])
        sanitized["error_code"] = code
        sanitized["error"] = message[: settings.job_error_max_chars]
    if "logs_short" in sanitized:
        sanitized["logs_short"] = _clean_text(
            sanitized["logs_short"], limit=settings.job_log_max_chars
        )
    return sanitized


def execute_tenant_job(func: Callable[..., Any], args: tuple[Any, ...]) -> Any:
    """Run an RQ function inside its tenant and prevent persisted tracebacks."""

    job = get_current_job()
    if job is None:
        return {"ok": False, "error_code": "tenant_context_missing"}
    try:
    ...
    with tenant_context(owner_hash):
        ...
        if isinstance(timeout, bool) or not isinstance(timeout, (int, float)) or timeout <= 0:
            timeout = settings.ffmpeg_timeout_seconds + 60
        try:
            refresh_job_admission(
                job.id,
                int(timeout) + 60,
        ...
        except (JobAdmissionError, OverflowError, ValueError):
            ...
            return {"ok": False, "error_code": "queue_unavailable"}
        try:
            with job_storage_budget():
                try:
                    return func(*args)
                except Exception as exc:
                    purge_current_job_outputs(owner_hash)
                    _finish_job(
                        job.id,
                        "error",
                        {"error": exc, "logs_short": None, "finished_at": utc_now_iso()},
                    )
                    return {"ok": False, "error_code": _safe_error(exc)[0]}
        finally:
            release_job_admission(job.id, owner_hash=owner_hash)


def release_tenant_job_admission_callback(
    job: Any,
    connection: Any,
    *exc_info: Any,
) -> None:
    """Idempotently reap admission after terminal RQ failures or stops."""

    del connection, exc_info
    meta = getattr(job, "meta", None)
    owner_hash = meta.get("owner_hash") if isinstance(meta, dict) else None
    if not isinstance(owner_hash, str):
        return
    with suppress(Exception):
        release_job_admission(str(job.id), owner_hash=owner_hash)
...
def _probe_or_error(path: str) -> dict[str, Any]:
    try:
        return validate_media_probe(run_ffprobe(path))
    except Exception as exc:
...
def _parse_loudnorm_json(logs: str) -> dict[str, Any] | None:
    ...
    for raw in reversed(candidates):
        ...
        if {"input_i", "input_tp", "input_lra"} & keys or {
            "measured_I",
            "measured_TP",
            "measured_LRA",
        } & keys:
...
def _caption_metrics(
    ...
    return {
        "caption_readability_score": round(avg_readability, 2)
...
def transcode_job(
    ...
    output_path = os.path.join(
        settings.storage_temp_dir, f"{uuid.uuid4().hex}{preset_def['output_ext']}"
    )
    logs = ""
    try:
        probe = _probe_or_error(input_path)
        try:
            validate_preset_geometry(probe, preset_def)
        except MediaLimitError as exc:
            raise JobError("Preset output geometry exceeds the configured limit") from exc
        output_kind = kind_from_mime_type(preset_def.get("mime_type"))
        if output_kind in {"image", "video"}:
            map_args = ["-map", f"0:{selected_video_stream_index(probe)}"]
            if output_kind == "video" and _has_audio_stream(probe):
                map_args += ["-map", f"0:{selected_audio_stream_index(probe)}"]
        elif output_kind == "audio":
            map_args = ["-map", f"0:{selected_audio_stream_index(probe)}"]
        else:
            raise JobError("Preset output media type is unsupported")
        args = ["-i", input_path, *map_args, *preset_def["ffmpeg_args"], output_path]
        logs = run_ffmpeg(args)
        update_job(job_id, {"progress": 80, "updated_at": utc_now_iso()})
        _enforce_output_size(output_path)
        output_probe = _probe_optional(output_path)
        if (
            output_probe
            and output_probe.get("duration_sec")
            and output_probe["duration_sec"] > settings.max_duration_seconds
        ):
...
def thumbnail_job(
    ...
    try:
        probe = _probe_or_error(input_path)
        try:
            validate_thumbnail_geometry(probe, width)
        except MediaLimitError as exc:
            raise JobError("Thumbnail geometry exceeds the configured limit") from exc
        args = [
            "-ss",
            str(time_sec),
            "-i",
            input_path,
            "-map",
            f"0:{selected_video_stream_index(probe)}",
            *vf,
            "-vframes",
            "1",
            output_path,
...
def extract_audio_job(
    ...
    logs = ""
    try:
        input_probe = _probe_or_error(input_path)
        try:
            audio_index = selected_audio_stream_index(input_probe)
        except MediaLimitError as exc:
            raise JobError("Input has no selectable audio stream") from exc
        args = ["-i", input_path, "-map", f"0:{audio_index}", "-vn"]
        if fmt == "mp3":
            args += ["-c:a", "libmp3lame"]
            if bitrate:
                args += ["-b:a", bitrate]
        ...
        if (
            probe
            and probe.get("duration_sec")
            and probe["duration_sec"] > settings.max_duration_seconds
        ):
            raise JobError("Output exceeds max duration")
        output_asset = _create_output_asset(
            output_path,
...
def trim_job(
    ...
    try:
        input_probe = _probe_or_error(input_path)
        try:
            map_args = ["-map", f"0:{selected_video_stream_index(input_probe)}"]
            if _has_audio_stream(input_probe):
                map_args += ["-map", f"0:{selected_audio_stream_index(input_probe)}"]
        except MediaLimitError as exc:
            raise JobError("Input has no selectable video stream") from exc
        args = [
            "-ss",
            str(start_sec),
            "-to",
...
def captions_burn_in_job(
    ...
    resolved_max_chars = (
        ...
        else (brand_kit.get("caption_max_chars") if brand_kit else None)
    )
    resolved_max_lines = (
        max_lines
        if max_lines is not None
        else (brand_kit.get("caption_max_lines") if brand_kit else None)
    )
    resolved_max_words = (
        max_words
        if max_words is not None
        else (brand_kit.get("caption_max_words") if brand_kit else None)
    )
    ...
    resolved_position = (
        position if position else (brand_kit.get("caption_position") if brand_kit else None)
    )
    ...
    resolved_font_size = (
        ...
        else (brand_kit.get("caption_font_size") if brand_kit else None)
    )
    resolved_font_size = sanitize_font_size(resolved_font_size, settings.caption_font_size)

    resolved_font_color = (
        font_color if font_color else (brand_kit.get("caption_text_color") if brand_kit else None)
    )
    resolved_font_color = sanitize_color(resolved_font_color, settings.caption_text_color)

    resolved_box_color = (
        box_color if box_color else (brand_kit.get("caption_box_color") if brand_kit else None)
    )
    ...
    resolved_box_opacity = (
        ...
        else (brand_kit.get("caption_box_opacity") if brand_kit else None)
    )
    resolved_box_opacity = (
        float(resolved_box_opacity)
        if resolved_box_opacity is not None
        else settings.caption_box_opacity
    )
    ...
    resolved_highlight = (
        highlight_color
        if highlight_color
        else (brand_kit.get("caption_highlight_color") if brand_kit else None)
    )
    ...
    resolved_padding = (
        ...
        else (brand_kit.get("caption_padding_px") if brand_kit else None)
    )
    ...
    resolved_safe_bottom = (
        safe_zone_bottom_px
        if safe_zone_bottom_px is not None
        else (
            profile_bottom
            if profile_bottom is not None
            else (brand_kit.get("caption_safe_zone_bottom_px") if brand_kit else None)
        )
    )
    resolved_safe_top = (
        safe_zone_top_px
        if safe_zone_top_px is not None
        else (
            profile_top
            if profile_top is not None
            else (brand_kit.get("caption_safe_zone_top_px") if brand_kit else None)
        )
    )
    ...
    resolved_font_name = (
        font_name if font_name else (brand_kit.get("caption_font_name") if brand_kit else None)
    )
    resolved_font_asset_id = (
        font_asset_id
        if font_asset_id
        else (brand_kit.get("caption_font_asset_id") if brand_kit else None)
    )
...
def video_add_logo_job(
    ...
    try:
        ...
        probe = _probe_or_error(input_path)
        try:
            logo_probe = validate_media_file(
                logo_path,
                expected_kind="image",
                byte_limit=max(settings.max_ingest_bytes, settings.max_output_bytes),
            )
        except MediaLimitError as exc:
            raise JobError("Logo media exceeds a configured limit") from exc
        video_stream = selected_video_stream(probe)
        width = video_stream.get("width")
        if not width:
            raise JobError("Unable to determine video width")
        logo_width = max(1, int(width * resolved_scale / 100))
        logo_stream = selected_video_stream(logo_probe)
        logo_source_width = logo_stream.get("width")
        logo_source_height = logo_stream.get("height")
        if not logo_source_width or not logo_source_height:
            raise JobError("Unable to determine logo geometry")
        try:
            logo_height = max(1, int(logo_width * logo_source_height / logo_source_width))
            validate_geometry(logo_width, logo_height, field="logo frame")
        except MediaLimitError as exc:
            raise JobError("Logo geometry exceeds the configured limit") from exc

        x_expr, y_expr = _logo_overlay_xy(resolved_position)
        logo_chain = (
            f"[1:{selected_video_stream_index(logo_probe)}]scale={logo_width}:-1,format=rgba"
        )
        if resolved_opacity < 1:
            logo_chain += f",colorchannelmixer=aa={resolved_opacity}"
        logo_chain += "[logo]"
        overlay_chain = (
            f"[0:{selected_video_stream_index(probe)}][logo]overlay={x_expr}:{y_expr}[v]"
        )
        ...
        if _has_audio_stream(probe):
            output_index = args.index("-c:v")
            args[output_index:output_index] = [
                "-map",
                f"0:{selected_audio_stream_index(probe)}",
            ]
        ...
        if (
            probe_out
            and probe_out.get("duration_sec")
            and probe_out["duration_sec"] > settings.max_duration_seconds
        ):
            raise JobError("Output exceeds max duration")
        ttl_seconds = settings.asset_ttl_seconds()
        output_asset = _create_output_asset(
...
def video_concat_job(
    ...
    try:
        for asset_id in asset_ids:
            ...
            try:
                durations.append(selected_video_duration(probe))
            except MediaLimitError as exc:
                raise JobError("Duration missing for concat input") from exc
            probes.append(probe)
        ...
        primary_stream = selected_video_stream(probes[0])
        width = int(target_width) if target_width else int(primary_stream.get("width") or 0)
        height = int(target_height) if target_height else int(primary_stream.get("height") or 0)
        try:
            width, height, _ = validate_concat_probe_plan(
                probes,
            ...
            raise JobError("Concat output exceeds a configured media limit") from exc

        args: list[str] = []
        for path, _ in input_paths:
            args += ["-i", path]

        audio_stream_specs: list[str] = []
        next_input_index = len(input_paths)
        for idx, probe in enumerate(probes):
            if include_audio and _has_audio_stream(probe):
                audio_stream_specs.append(f"{idx}:{selected_audio_stream_index(probe)}")
                continue
            if include_audio:
                args += [
                    "-f",
                ...
                audio_stream_specs.append(f"{next_input_index}:a")
                next_input_index += 1
            else:
        ...
        for idx, duration in enumerate(durations):
            v_label = f"v{idx}"
            v_chain = (
                f"[{idx}:{selected_video_stream_index(probes[idx])}]"
            ...
            if include_audio:
                audio_stream_spec = audio_stream_specs[idx]
                a_label = f"a{idx}"
                a_chain = (
                    f"[{audio_stream_spec}]atrim=duration={duration},asetpts=PTS-STARTPTS,"
        ...
        if transition == "none":
            ...
            if include_audio:
                concat_inputs = "".join([f"[v{idx}][a{idx}]" for idx in range(len(asset_ids))])
                filter_parts.append(f"{concat_inputs}concat=n={len(asset_ids)}:v=1:a=1[vout][aout]")
            else:
...
def image_to_video_job(
    ...
    duration_sec = float(
        duration_sec if duration_sec is not None else settings.default_image_duration_sec
    )
    if duration_sec <= 0:
    ...
    try:
        ...
        try:
            validate_planned_video_work(
                ...
                field="image video output",
            )
        except MediaLimitError as exc:
            raise JobError("Image video output exceeds a configured media limit") from exc
        vf = (
            f"scale={width}:{height}:force_original_aspect_ratio=decrease,"
        ...
        args = [
            ...
            input_path,
            "-map",
            f"0:{selected_video_stream_index(input_probe)}",
            "-t",
            str(duration_sec),
            "-vf",
            vf,
...
def images_to_slideshow_job(
    ...
    resolved_durations = (
        ...
        else [duration_per_image] * len(asset_ids)
    )
    ...
    try:
        for asset_id in asset_ids:
            ...
            path, cleanup = _resolve_input_path(asset)
            input_paths.append((path, cleanup))
            input_probes.append(_probe_or_error(path))
        ...
        try:
            ...
            raise JobError("Slideshow output exceeds a configured media limit") from exc
        ...
        for idx, duration in enumerate(resolved_durations):
            v_label = f"v{idx}"
            v_chain = (
                f"[{idx}:{selected_video_stream_index(input_probes[idx])}]"
                f"scale={width}:{height}:force_original_aspect_ratio=decrease,"
                f"pad={width}:{height}:(ow-iw)/2:(oh-ih)/2:color={pad_color},"
...
def images_to_slideshow_ken_burns_job(
    ...
    job_id = job_id_override if job_id_override is not None else (job.id if job else "")
    start_ts = job_timer()
    _log_job_started("images_to_slideshow_ken_burns", job_id, asset_ids[0] if asset_ids else None)
    _finish_job(
        job_id,
        "running",
        {"started_at": utc_now_iso(), "progress": 10, "cache_key": cache_key},
    ...
    try:
        ...
        try:
            validate_planned_video_work(
                ...
                fps=fps,
                duration=total_duration,
                field="Ken Burns slideshow output",
            )
        except MediaLimitError as exc:
            raise JobError("Slideshow output exceeds a configured media limit") from exc
...
def audio_mix_job(
    ...
    try:
        ...
        for idx in range(len(asset_ids)):
            label = f"a{idx}"
            chain = (
                f"[{idx}:{selected_audio_stream_index(input_probes[idx])}]"
                f"aresample={settings.audio_sample_rate},"
                "aformat=sample_fmts=fltp:channel_layouts=stereo"
            )
...
def audio_duck_job(
    ...
    try:
        ...
        inputs.append((voice_path, voice_cleanup))
        music_path, music_cleanup = _resolve_input_path(music_asset)
        inputs.append((music_path, music_cleanup))
        ...
        filter_complex = (
            f"[0:{selected_audio_stream_index(music_probe)}]"
            f"aresample={settings.audio_sample_rate},"
            "aformat=sample_fmts=fltp:channel_layouts=stereo,"
            f"volume={music_gain}[music];"
            f"[1:{selected_audio_stream_index(voice_probe)}]"
            f"aresample={settings.audio_sample_rate},"
...
def audio_fade_job(
    ...
    try:
        ...
        fade_in_sec = (
            float(fade_in_sec) if fade_in_sec is not None else settings.audio_fade_default_sec
        )
        fade_out_sec = (
            float(fade_out_sec) if fade_out_sec is not None else settings.audio_fade_default_sec
        )
...
def video_replace_audio_job(
    ...
    try:
        ...
        input_paths.append((video_path, video_cleanup))
        audio_path, audio_cleanup = _resolve_input_path(audio_asset)
        input_paths.append((audio_path, audio_cleanup))
        output_path = os.path.join(settings.storage_temp_dir, f"{uuid.uuid4().hex}.mp4")
        video_probe = _probe_or_error(video_path)
        audio_probe = _probe_or_error(audio_path)
        use_shortest = True
        ...
        args = [
            ...
            audio_path,
            "-map",
            f"0:{selected_video_stream_index(video_probe)}",
            "-map",
            f"1:{selected_audio_stream_index(audio_probe)}",
            "-c:v",
            "copy",
            "-c:a",
            "aac",
...
def video_analyze_job(
    ...
    try:
        input_path, cleanup = _resolve_input_path(asset)
        input_paths.append((input_path, cleanup))
        if reference_asset_id:
            ...
            input_paths.append((reference_path, reference_cleanup))

        def collect_metrics(
            path: str, size_bytes: int | None, include_clipping: bool
        ) -> dict[str, Any]:
            probe = _probe_or_error(path)
            duration = probe.get("duration_sec")
            ...
            if has_audio:
                try:
                    ...
                    for key, fallback in [
                        ...
                        raw_value: Any = (
                            payload.get(key) if key in payload else payload.get(fallback)
                        )
                        try:
        ...
        include_clipping = bool(
            rubric and rubric.get("weights", {}).get("audio.clipping_pct", 0) > 0
        )
        ...
        for layer in layers:
            ...
            if missing_vars:
                if optional_layer:
                    continue
                raise JobError(
                    f"Missing template variables: {', '.join(sorted(set(missing_vars)))}"
                )
            ...
            if (
                layer_type == "logo"
                and not params.get("logo_asset_id")
                and not params.get("logo_key")
            ):
                if optional_layer:
                    continue
...
def _required_workflow_string(value: Any, field: str, node_type: str) -> str:
    if not isinstance(value, str) or not value:
        raise JobError(f"{node_type} requires {field}")
    return value


def _workflow_float(value: Any, default: float, field: str, node_type: str) -> float:
    if value is None:
        return default
    try:
        return float(value)
    except (TypeError, ValueError) as exc:
        raise JobError(f"{node_type} requires numeric {field}") from exc
...
def workflow_job(
    ...
    for node in nodes:
        if not isinstance(node, dict):
            raise JobError("Each workflow node must be an object")
        node_id = node.get("id")
        if not isinstance(node_id, str) or not node_id or node_id in remaining:
            raise JobError("Each workflow node requires a unique non-empty string id")
            ...
            for node_id, node in list(remaining.items()):
                ...
                if input_ref is not None:
                    if not isinstance(input_ref, str) or not input_ref:
                        raise JobError("workflow node input must be a non-empty string")
                    deps = [input_ref]
                if inputs_ref is not None:
                    if not isinstance(inputs_ref, list):
                        raise JobError("inputs must be a list")
                    if any(not isinstance(item, str) or not item for item in inputs_ref):
                        raise JobError("workflow node inputs must contain non-empty strings")
                    deps = list(inputs_ref)
                ...
                if input_ref is not None:
                    resolved_input = outputs.get(input_ref, input_ref)
                if inputs_ref is not None:
                ...
                input_asset_id = ""
                if node_type in _WORKFLOW_SINGLE_INPUT_TYPES:
                    input_asset_id = _required_workflow_string(resolved_input, "input", node_type)
                if (
                    node_type
                    in {
                        "video_concat",
                ...
                if cached_output:
                    ...
                    if node_type == "transcode":
                        output_asset = transcode_job(
                            input_asset_id,
                            _required_workflow_string(params.get("preset"), "preset", node_type),
                        ...
                        output_asset = video_add_text_job(
                            input_asset_id,
                            _required_workflow_string(params.get("text"), "text", node_type),
                            params.get("position"),
                            params.get("font_size"),
                            params.get("font_color"),
                            params.get("background_box"),
                        ...
                        output_asset = image_to_video_job(
                            ...
                            _workflow_float(
                                ...
                                settings.default_image_duration_sec,
                                "duration_sec",
                                node_type,
                            ),
                            params.get("width"),
                        ...
                        output_asset = audio_duck_job(
                            _required_workflow_string(voice_ref, "voice_asset_id", node_type),
                            _required_workflow_string(music_ref, "music_asset_id", node_type),
                            params.get("output_format", "m4a"),
                            params.get("ratio"),
                        ...
                        output_asset = template_apply_job(
                            input_asset_id,
                            _required_workflow_string(
                                params.get("template_name"), "template_name", node_type
                            ),
                        ...
                        output_asset = brand_kit_apply_job(
                            input_asset_id,
                            _required_workflow_string(
                                params.get("brand_kit_id"), "brand_kit_id", node_type
                            ),
                            params.get("text"),
                            params.get("position"),
                            cache_key=node_cache_key,
...
def _primary_variant_preset(quality: str, framing_mode: str) -> str:
    if framing_mode == "crop":
        return (
            ...
            if quality == "draft"
            else "mp4_social_vertical_1080x1920"
        )
    return (
        "mp4_social_vertical_720x1280_safe_pad"
        if quality == "draft"
        else "mp4_social_vertical_1080x1920_safe_pad"
    )
...
def render_iterate_job(
    ...
    try:
        ...
        caption_position = caption_position or (
            brand_kit.get("caption_position") if brand_kit else None
        )
        resolved_caption_font_size = (
            caption_font_size
            if caption_font_size is not None
            else (brand_kit.get("caption_font_size") if brand_kit else settings.caption_font_size)
        )
        caption_font_size = int(
            settings.caption_font_size
            if resolved_caption_font_size is None
            else resolved_caption_font_size
        )
        resolved_caption_padding_px = (
            caption_padding_px
            if caption_padding_px is not None
            else (brand_kit.get("caption_padding_px") if brand_kit else settings.caption_padding_px)
        )
        caption_padding_px = int(
            settings.caption_padding_px
            if resolved_caption_padding_px is None
            else resolved_caption_padding_px
        )
        resolved_caption_max_chars = (
            caption_max_chars
            if caption_max_chars is not None
            else (brand_kit.get("caption_max_chars") if brand_kit else settings.caption_max_chars)
        )
        caption_max_chars = int(
            settings.caption_max_chars
            if resolved_caption_max_chars is None
            else resolved_caption_max_chars
        )
        resolved_caption_max_lines = (
            caption_max_lines
            if caption_max_lines is not None
            else (brand_kit.get("caption_max_lines") if brand_kit else settings.caption_max_lines)
        )
        caption_max_lines = int(
            settings.caption_max_lines
            if resolved_caption_max_lines is None
            else resolved_caption_max_lines
        )
        resolved_caption_max_words = (
            caption_max_words
            if caption_max_words is not None
            else (brand_kit.get("caption_max_words") if brand_kit else settings.caption_max_words)
        )
        caption_max_words = int(
            settings.caption_max_words
            if resolved_caption_max_words is None
            else resolved_caption_max_words
        )
        profile_bottom, profile_top = resolve_safe_zone_profile(caption_safe_zone_profile)
        resolved_safe_zone_bottom_px = (
            caption_safe_zone_bottom_px
            if caption_safe_zone_bottom_px is not None
            else (
                profile_bottom
                if profile_bottom is not None
                else (
                    brand_kit.get("caption_safe_zone_bottom_px")
                    if brand_kit
                    else settings.caption_safe_zone_bottom_px
                )
            )
        )
        caption_safe_zone_bottom_px = int(
            settings.caption_safe_zone_bottom_px
            if resolved_safe_zone_bottom_px is None
            else resolved_safe_zone_bottom_px
        )
        resolved_safe_zone_top_px = (
            caption_safe_zone_top_px
            if caption_safe_zone_top_px is not None
            else (
                profile_top
                if profile_top is not None
                else (
                    brand_kit.get("caption_safe_zone_top_px")
                    if brand_kit
                    else settings.caption_safe_zone_top_px
                )
            )
        )
        caption_safe_zone_top_px = int(
            settings.caption_safe_zone_top_px
            if resolved_safe_zone_top_px is None
            else resolved_safe_zone_top_px
        )

        resolved_caption_box_opacity = (
            caption_box_opacity
            if caption_box_opacity is not None
            else (
                brand_kit.get("caption_box_opacity") if brand_kit else settings.caption_box_opacity
            )
        )
        caption_box_opacity = float(
            settings.caption_box_opacity
            if resolved_caption_box_opacity is None
            else resolved_caption_box_opacity
        )

        audio_target_lufs = float(
            settings.audio_norm_i if audio_target_lufs is None else audio_target_lufs
        )
        audio_lra = float(settings.audio_norm_lra if audio_lra is None else audio_lra)
        audio_true_peak = float(
            settings.audio_norm_tp if audio_true_peak is None else audio_true_peak
        )
        ducking_ratio = float(settings.ducking_ratio if ducking_ratio is None else ducking_ratio)
        ducking_threshold = float(
            settings.ducking_threshold if ducking_threshold is None else ducking_threshold
        )
        ducking_attack_ms = int(
            settings.ducking_attack_ms if ducking_attack_ms is None else ducking_attack_ms
        )
        ducking_release_ms = int(
            settings.ducking_release_ms if ducking_release_ms is None else ducking_release_ms
        )
        music_gain = float(settings.ducking_music_gain if music_gain is None else music_gain)

        trim_silence = bool(trim_silence) if trim_silence is not None else False
        trim_silence_min_sec = float(
            settings.audio_min_silence_sec if trim_silence_min_sec is None else trim_silence_min_sec
        )
        trim_silence_threshold_db = float(
            settings.audio_silence_db
            if trim_silence_threshold_db is None
            else trim_silence_threshold_db
        )
        ...
        caption_font_size_min = (
            int(caption_font_size_min)
            if caption_font_size_min is not None
            else settings.auto_caption_font_size_min
        )
        caption_font_size_max = (
            int(caption_font_size_max)
            if caption_font_size_max is not None
            else settings.auto_caption_font_size_max
        )
        caption_box_opacity_min = (
            float(caption_box_opacity_min)
            if caption_box_opacity_min is not None
            else settings.auto_caption_box_opacity_min
        )
        caption_box_opacity_max = (
            float(caption_box_opacity_max)
            if caption_box_opacity_max is not None
            else settings.auto_caption_box_opacity_max
        )
        music_gain_min = (
            float(music_gain_min) if music_gain_min is not None else settings.auto_music_gain_min
        )
        music_gain_max = (
            float(music_gain_max) if music_gain_max is not None else settings.auto_music_gain_max
        )
        max_crop_pct = (
            float(max_crop_pct) if max_crop_pct is not None else settings.auto_max_crop_pct
        )
        min_duration_sec = (
            float(min_duration_sec)
            if min_duration_sec is not None
            else settings.auto_min_duration_sec
        )
        ...
        caption_font_size = int(
            min(max(caption_font_size, caption_font_size_min), caption_font_size_max)
        )
        caption_box_opacity = float(
            min(max(caption_box_opacity, caption_box_opacity_min), caption_box_opacity_max)
        )
        ...
        threshold = (
            ...
            else float(rubric.get("pass_threshold", 85))
        )
        targets: dict[str, Any] = rubric.get("targets", {})
            ...
            if max_crop_pct <= 0:
                ...
                try:
                    ...
                    if probe and probe.get("width") and probe.get("height"):
                        crop_pct = _estimate_crop_pct(
                            int(probe["width"]), int(probe["height"]), 9.0 / 16.0
                        )
        ...
        for idx in range(max_iterations):
            ...
            primary_variant: dict[str, Any] = outputs.get("9x16") or next(
                iter(outputs.values()), {}
            )
            eval_asset_id = None
            if captions_srt or captions_vtt or words_json:
            ...
            if fail_fast:
                ...
                if (
                    ...
                    and float(duration_sec) < min_duration_sec
                ):
                ...
                if (captions_srt or captions_vtt or words_json) and caption_metrics.get(
                    "segment_count"
                ) in {0, None}:
            ...
            if not lock_captions:
                if caption_metrics.get("safe_zone_violations"):
                    caption_needs = True
                if readability is not None and readability < targets.get(
            ...
            if not lock_audio:
                ...
                if loudness is not None and (
                    loudness < target_lufs - tolerance or loudness > target_lufs + tolerance
                ):
                    audio_needs = True
            ...
            def apply_caption_adjustments(
                ...
                if (
                    caption_max_chars is None
                    or caption_max_words is None
                    or caption_max_lines is None
                    or caption_font_size is None
                    or caption_box_opacity is None
                ):
                    raise JobError("Resolved caption iteration settings are incomplete")
                if caption_metrics.get("safe_zone_violations"):
                    caption_safe_zone_bottom_px = (
                        int(caption_safe_zone_bottom_px or settings.caption_safe_zone_bottom_px)
                        + 16
                    )
                if readability is not None and readability < targets.get(
                    "caption_readability_min", 70.0
            ...
            def apply_audio_adjustments(
                ...
                if (
                    audio_target_lufs is None
                    or audio_true_peak is None
                    or music_gain is None
                    or trim_silence_min_sec is None
                    or trim_silence_threshold_db is None
                ):
                    raise JobError("Resolved audio iteration settings are incomplete")
                target_lufs = float(targets.get("loudness_lufs", -16.0))
                tolerance = float(targets.get("loudness_tolerance", 2.5))
                ...
                if true_peak is not None and true_peak > targets.get("true_peak_max_db", -1.5):
                    audio_true_peak = min(
                        float(audio_true_peak), targets.get("true_peak_max_db", -1.5)
                    )
            ...
            def apply_framing_adjustments(framing_needs=framing_needs) -> None:
        ...
        _finish_job(
            ...
            "success",
            {
                "output_asset_ids": [best_result.get("analysis", {}).get("asset_id")]
                if best_result
                else [],
