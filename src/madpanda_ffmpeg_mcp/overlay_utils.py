...
def sanitize_box_border(value: int | None, default: int) -> int:
    ...
    if width < 0 or width > settings.max_box_border_width:
        raise ValueError(f"box_border_width must be between 0 and {settings.max_box_border_width}")
    return width
...
def sanitize_scale_pct(value: int | None, default: int) -> int:
    ...
    if pct < settings.logo_min_scale_pct or pct > settings.logo_max_scale_pct:
        raise ValueError(
            "scale_pct must be between "
            f"{settings.logo_min_scale_pct} and {settings.logo_max_scale_pct}"
        )
    return pct
...
def sanitize_opacity(value: float | None, default: float) -> float:
    ...
    if not math.isfinite(opacity) or opacity < 0 or opacity > settings.logo_max_opacity:
...
def escape_drawtext_value(value: str) -> str:
    if not isinstance(value, str) or "\x00" in value:
        raise ValueError("drawtext value contains invalid characters")
    escaped: list[str] = []
    for char in value:
        if char == "\n":
            escaped.append("\\n")
...
def resolve_logo_path(logo_asset_id: str | None, logo_key: str | None) -> tuple[str, bool]:
    if logo_asset_id:
