...
METRIC_PREFIX = "metrics:tenant:"
_METRIC_WARNING_INTERVAL_SECONDS = 60.0
_last_metric_warning = 0.0


def _metric_key(name: str) -> str:
    return f"{METRIC_PREFIX}{require_owner_hash()}:{name}"


def _warn_metrics_unavailable() -> None:
    """Rate-limit telemetry failure logs so an outage cannot amplify traffic."""

    global _last_metric_warning
    now = time.monotonic()
    if now - _last_metric_warning < _METRIC_WARNING_INTERVAL_SECONDS:
        return
    _last_metric_warning = now
    logger.warning("tenant metrics temporarily unavailable")
...
def _safe_incr(key: str, amount: int | float) -> None:
    try:
        ...
        pipeline.expire(key, settings.metrics_ttl_seconds)
        pipeline.execute()
    except Exception:
        _warn_metrics_unavailable()
        return
...
def collect_metrics_snapshot() -> dict[str, Any]:
    client = get_redis()
    owner_prefix = f"{METRIC_PREFIX}{require_owner_hash()}:"
    ...
    for key in client.scan_iter(match=f"{owner_prefix}*"):
        name = key[len(owner_prefix) :]
        raw = client.get(key)
        if raw is None:
            continue
        try:
