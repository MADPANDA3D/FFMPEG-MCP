...
class ToolManifestTests(unittest.TestCase):
    def test_manifest_is_lossless_complete_and_registry_owned(self) -> None:
        ...
        for descriptor in manifest["tools"]:
            ...
            self.assertTrue(
                all(
                    ...
                    for value in descriptor["annotations"].values()
                )
            )
            ...
            self.assertEqual(
                descriptor["descriptorHash"], hashlib.sha256(encoded).hexdigest()
            )
        ...
        self.assertEqual(
            manifest["descriptorHash"], hashlib.sha256(encoded_tools).hexdigest()
        )

        capability = next(
            item
            for item in manifest["tools"]
            if item["nativeToolName"] == "list_capabilities"
        )
        self.assertEqual(
    ...
    def test_descriptor_hash_excludes_runtime_build_sha(self) -> None:
        with patch.dict(os.environ, {"BUILD_SHA": "abcdef1234567"}, clear=False):
            first = build_tool_manifest(server.TOOL_REGISTRY)
        with patch.dict(os.environ, {"BUILD_SHA": "7654321fedcba"}, clear=False):
...
class NavigationToolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_configuration_readback_contains_presence_only(self) -> None:
        ...
        if server.settings.portal_grant_secret:
            self.assertNotIn(server.settings.portal_grant_secret, serialized)
        if server.settings.discord_bot_token:
            self.assertNotIn(server.settings.discord_bot_token, serialized)
        self.assertEqual(
            set(result), {"ok", "serviceId", "required", "optional", "missing"}
        )
        self.assertTrue(
            all(isinstance(value, bool) for value in result["required"].values())
        )
        self.assertTrue(
            all(isinstance(value, bool) for value in result["optional"].values())
        )
