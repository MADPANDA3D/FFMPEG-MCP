...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "brand_kit_delete": (
        ...
        "must require the exact confirmation phrase before execution; media assets "
        "already rendered with the kit are not deleted."
    ),
    "brand_kit_apply": (
    ...
    "media_export_to_drive": (
        ...
        "and return its Drive file ID. It writes to an external system and requires "
        "Drive configuration."
    ),
...
PARAMETER_DESCRIPTIONS: dict[str, str] = {
    "include_descriptors": "When true, include the complete ordered ToolManifest descriptors at the response top level.",
    "query": "Natural-language task or tool terms; all normalized tokens must match the ranked descriptor text.",
    ...
    "words_json": "Timed word objects containing word, start, and end values; provide only one caption source.",
    ...
    "trim_silence_min_sec": "Minimum silence duration in seconds that the render pipeline may trim.",
    ...
    "workflow": "Bounded workflow object containing approved nodes, dependencies, and output node IDs.",
...
def _title(name: str) -> str:
    return " ".join(
        part.upper() if part in {"url", "qa"} else part.capitalize()
...
def _parameter_description(name: str, tool_name: str) -> str:
    ...
    return f"{human.capitalize()} used by {_title(tool_name)}; omit it to use the documented server default."
...
def _output_schema(name: str) -> dict[str, Any]:
    ...
    if name in JOB_OUTPUT_TOOLS:
        properties = {
            "job_id": _property(
                "string", "Stable ID used with the job readback tools."
            ),
            "cache_hit": _property(
                "boolean", "Whether a matching completed result was reused."
            ),
        ...
        properties = {
            ...
            "expires_at": _property(
                "integer", "Unix expiry timestamp for the staged asset."
            ),
        }
        required = ["asset_id"]
        ...
        properties = {
            ...
            "report": _object_property(
                "Completed analysis report when available from cache."
            ),
        }
        required = ["job_id", "cache_hit"]
        ...
        properties = {
            ...
            "cache_hit": _property(
                "boolean", "Whether a cached iterative result was reused."
            ),
            "result": _object_property(
                "Completed iteration result when available from cache."
            ),
        }
        required = ["job_id", "cache_hit"]
        ...
        properties = {
            "template": _object_property("Complete approved template definition.")
        }
        required = ["template"]
    elif name in {"brand_kit_upsert", "brand_kit_get"}:
        properties = {
            "brand_kit": _object_property("Sanitized local brand-kit definition.")
        }
        required = ["brand_kit"]
        ...
        properties = {
            "deleted": _property(
                "boolean", "Whether the brand-kit definition was removed."
            )
        }
        required = ["deleted"]
        ...
        properties = {
            "preset": _object_property("Complete approved preset definition.")
        }
        required = ["preset"]
        ...
        properties = {
            "rubric": _object_property("Complete approved rubric definition.")
        }
        required = ["rubric"]
    elif name == "ffmpeg_capabilities":
        properties = {
            "tool_mode": _property("string", "Active individual or router tool mode."),
        ...
        properties = {
            "ok": _property(
                "boolean", "Whether required FFmpeg service configuration is ready."
            ),
            "serviceId": _property("string", "Canonical provider service ID."),
            "required": _object_property(
                "Required configuration presence booleans only."
            ),
        ...
        properties = {
            ...
            "entries": _property(
                ...
                items={
                    "type": "object",
                    "properties": {
                        "capability": _property(
                            "string", "Manifest capability category."
                        ),
                        "nativeToolName": _property(
                            "string", "Native FFmpeg MCP tool name."
                        ),
                        ...
                        "providerInterface": _property(
                            "string", "Underlying provider interface."
                        ),
                        "documentationUrl": _property(
                            "string", "Tool documentation URL."
                        ),
                    },
                    "required": [
                        "capability",
        ...
        properties = {
            "tool": _object_property("Complete lossless ToolManifest descriptor.")
        }
        required = ["tool"]
        ...
        properties = {
            ...
            "results": _property(
                ...
                items={
                    ...
                    "properties": {
                        "serviceId": _property(
                            "string", "Canonical provider service ID."
                        ),
                        ...
                        "risk": _property(
                            "string", "Normalized read, write, or destructive risk."
                        ),
                        "tier": _property("string", "Contract tier."),
                        "score": _property(
                            "integer", "Deterministic text-match score."
                        ),
                    },
                    "required": [
                        "serviceId",
        ...
        properties = {
            ...
            "progress": _property("integer", "Progress percent when known."),
            "progress_pct": _property("integer", "Compatibility alias for progress."),
            "output_asset_ids": _property(
                "array", "Completed output asset IDs.", items={"type": "string"}
            ),
            "report": _object_property("Optional analysis report."),
            "ranking": _property(
                "array",
                "Optional comparison ranking.",
                items={"type": "object", "additionalProperties": True},
            ),
            "result": _object_property("Optional workflow or iteration result."),
            "qa": _object_property("Normalized QA outcome."),
            "error": _property("string", "Safe processing error summary when failed."),
            "logs_short": _property("string", "Bounded sanitized job log summary."),
            "last_log_line": _property("string", "Last non-empty sanitized log line."),
            "error_code": _property("string", "Normalized recoverable error category."),
            "started_at": _property("string", "UTC start timestamp when known."),
            "finished_at": _property("string", "UTC completion timestamp when known."),
            "cache_hit": _property(
                "boolean", "Whether the job result came from cache."
            ),
        }
        ...
        properties = {
            ...
            "status": _property("string", "Provider job status."),
            "progress_pct": _property("integer", "Progress percent when known."),
            "phase": _property(
                "string", "Current bounded processing phase when known."
            ),
        }
        ...
        properties = {
            ...
            "logs_short": _property("string", "Bounded sanitized job log summary."),
            "last_log_line": _property("string", "Last non-empty sanitized log line."),
            "error": _property("string", "Safe error summary when the job failed."),
        }
        required = ["job_id", "status"]
    elif name == "metrics_snapshot":
        properties = {
            "queue_depth": _object_property(
                "Current queue depth keyed by configured queue name."
            )
        }
        ...
        properties = {
            ...
            "expires_at": _property(
                "integer", "Unix timestamp at which the URL expires."
            ),
        }
        required = ["url", "expires_at"]
    elif name == "media_export_to_drive":
        properties = {
            "drive_file_id": _property("string", "Created Google Drive file ID.")
        }
...
def _build_sha() -> str:
    for name in ("BUILD_SHA", "GIT_COMMIT_SHA", "SOURCE_VERSION", "MCP_BUILD_SHA"):
...
def build_tool_manifest(
    ...
    if registry_names != metadata_names:
        ...
        raise ValueError(
            f"ToolManifest registry drift; missing={missing}, stale={stale}"
        )
    descriptors = [
        _descriptor(name, tool_functions[name]) for name in sorted(tool_functions)
    ]
    counts = {
        "raw": len(descriptors),
...
def resolve_tool_descriptor(
    manifest: Mapping[str, Any], requested_name: str
) -> dict[str, Any]:
...
def _tokens(value: str) -> list[str]:
    ...
    return [
        part for part in re.sub(r"[^a-z0-9]+", " ", normalized.lower()).split() if part
    ]
...
def capability_groups(manifest: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        {"category": category, "tools": grouped[category]}
