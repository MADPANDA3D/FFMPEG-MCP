...
5. In Portal mode, set the existing private broker network and configure the broker to inject the grant and stable `X-MADPANDA-PORTAL-SUBJECT` after stripping caller-supplied protected headers. For `media_export_to_discord`, the broker must also inject only that caller's Discord BYOK token through `MCP_DISCORD_TOKEN_HEADER`.
6. If using S3, Drive, or Discord, opt in explicitly. Standalone mode must grant only the exact file, folder, or channel IDs required. Portal-mode Discord export uses the caller's bot permissions and numeric destination channel instead of a shared token or global channel allowlist. Mount Drive credentials read-only with `examples/docker-compose.drive.override.yml`.
